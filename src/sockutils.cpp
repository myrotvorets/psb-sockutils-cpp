...
listening_socket_t create_listening_socket(const std::string& address, std::uint16_t port, const socket_options_t& opts)
{
    ...
    int sock     = socket(is_ipv6 ? AF_INET6 : AF_INET, SOCK_STREAM, IPPROTO_TCP);
...
#pragma clang unsafe_buffer_usage begin
            std::string_view abstract_socket_name(&addr.sun_path[1], len - offsetof(sockaddr_un, sun_path) - 1);
